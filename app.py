...
# SECURITY: This signs the session cookie. Use a complex random string.
...
DBDetails = {
    ...
    'port': '6543',
    'database': 'postgres',
    'password': '@Visionscan31', # In production, use os.getenv('DB_PASS')
...
def login():
    ...
    try:
        ...
        if result and str(result[0]) == str(phonenumber):
            # AUTHENTICATION SUCCESS: Save user in session
            session['user'] = name
...
@app.route('/camera')
def camera_page():
    # SECURE ENDPOINT: Check if 'user' is logged in
...
def scan():
    # Only allow scanning if logged in
    if 'user' not in session:
    ...
    if len(faces) > 0:
        for (x, y, w, h) in faces:
            ...
            break # Just process the first face for speed
